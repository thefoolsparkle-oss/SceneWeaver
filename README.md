...
搜索页支持本地日期范围。关键词和已安装 CLIP 的语义候选都会由 SQLite 作为硬条件筛选，参考图、相似图与 Entity 结果也会在页面统一按相同范围过滤。对视频和音频，FFprobe 返回的 RFC 3339 容器创建时间会优先用于筛选；没有该元数据、FFmpeg 不可用或图片尚未提取 EXIF 时，安全回退到本地文件修改时间，绝不把文件日期伪称为拍摄日期。
