...
> 首次启动时，首页会在无素材库状态提供三步引导：选择文件夹、选择快速/平衡/精确索引模式，然后创建素材库并开始后台扫描。源素材不会被复制、移动或上传。
...
当前可用闭环包括：本地文件夹素材库、可恢复扫描任务、图片缩略图、基础关键词 / Must / Must Not 搜索、收藏、默认选片与可编辑选片集合（标签、备注、评分、推荐入/出点、拖拽排序）。默认和自定义集合都可导出 CSV、JSON、EDL、FCPXML、PNG 及离线 HTML 联系表；片段范围会被保留，已填写的推荐入/出点会优先覆盖片段范围。视频在扫描时会自动切分镜头并生成关键帧、短预览与基础黑帧/模糊质量指标（需要本机 FFmpeg）；关键帧还会以保守的下部高对比度启发式写入字幕提示。双下角同时出现小面积高对比 HUD 元素时还会写入保守 UI 提示。因此“字幕 / subtitle”“UI / HUD”“黑帧 / 黑屏”“模糊 / blur”可作为视频的 Must / Should / Must Not 条件；黑帧比例达到 85%、模糊分达到 80% 时才会命中后两类提示。多个片段条件会约束同一个镜头：例如“字幕且不要模糊”不会错误地由两个不同片段拼凑满足；“不要字幕”会保留同一视频中存在的无字幕片段。搜索结果会标出已定位的片段数，打开视频片段面板时会直接聚焦这些镜头，随后可预览、复制入点时间码或将单个片段加入默认或任一自定义 Selects 集合。它们是单帧启发式，不是 OCR、通用 UI 识别或真实画质评测。FFmpeg 缺失或派生失败时仍保留可检索的视频素材和完整片段，并可稍后手动重试。超时的镜头检测或派生进程会被终止并回收，避免遗留后台 FFmpeg 占用扫描流程。准确片段可直接加入选片集合。设置页提供明确触发的本地 CLIP 模型下载与重建语义索引；参考图相似检索与 Entity 的正/负参考、候选“是/非实体”本地反馈已可用。中文通用视觉语义与完整 ACG 场景理解仍在开发中，详见 [`docs/KNOWN_LIMITATIONS.md`](./docs/KNOWN_LIMITATIONS.md)。

选片集合支持按标签筛选、多选后批量添加标签或批量打开源素材；打开操作只接受已入库资产 ID，并会对同一素材去重。

媒体卡片可复制完整路径或相对素材库根目录的路径；相对路径由后端按资产 ID 计算，不接受页面传入的目录。

搜索结果支持本地“隐藏”反馈：隐藏项不会再显示在关键词、相似图、参考图或 Entity 结果中，可随时一键恢复全部；该偏好仅写入 SQLite，不修改、移动或删除源素材。

搜索页支持本地日期范围。关键词和已安装 CLIP 的语义候选都会由 SQLite 作为硬条件筛选，参考图、相似图与 Entity 结果也会在页面统一按相同范围过滤。视频和音频优先使用 FFprobe 返回的 RFC 3339 容器创建时间；图片优先使用带 OffsetTime 的 EXIF DateTimeOriginal / DateTimeDigitized / DateTime。没有可信时区、元数据缺失或处理器不可用时，安全回退到本地文件修改时间，绝不把文件日期伪称为拍摄日期。

在实体页确认候选为“是实体”或“非实体”后，样本只写入本地 SQLite，并会立即重新检索当前实体候选；这不是经校准的身份置信度。

FFprobe 的本地元数据读取有 30 秒监督上限；超时会终止并回收子进程，单个异常媒体不会让后台扫描遗留探测进程。

媒体卡片会显示用于日期筛选的实际本地来源：可信拍摄/创建时间优先，否则明确显示文件修改时间。
...
实体结果卡片会说明候选来自本地实体名称、别名或正参考图匹配；它是来源解释而非经过真实数据集校准的身份置信度。

隔离桌面 E2E 已实际从实体页添加一张正参考图和一张负参考图，验证其 SQLite 持久化、结果区渲染以及负参考的本地排序惩罚。该验证使用确定性的颜色特征，不代表通用人物或角色识别准确率。
...
隔离桌面 E2E 已实际点击“参考图”，以不同颜色的本地 PNG 参考图召回已索引的对应素材。为避免自动化控制系统文件选择器，仅开发构建的测试会显式注入临时路径；production 构建始终保留用户选择文件的系统对话框。

隔离桌面 E2E 已从自定义集合的片段卡片保存推荐入/出点，再以真实按钮导出 CSV、JSON、EDL 和 FCPXML 并读取结果。JSON 输出最终生效范围，EDL 固定 30fps 非丢帧；仍需在目标专业软件中以真实媒体完成导入验证。

同一 E2E 还会通过真实“下移”按钮重新排序自定义集合，读取 SQLite 位置并检查后续 JSON 导出数组顺序，保证排序可持久化到交换文件。
