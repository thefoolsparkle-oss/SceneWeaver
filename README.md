...
桌面壳层另有 `npm.cmd run test:e2e`：它构建前端、以 `webdriver-e2e` feature 启动临时 debug SceneWeaver，启动本地 Vite 后直接连接应用内嵌 WebDriver，验证真实窗口标题、首页渲染、进入搜索页的导航，以及创建自定义选片集合后刷新仍由隔离 SQLite 保留。每次运行使用并清理独立临时应用数据目录，不触碰用户数据库。该链路不下载或使用 Edge/Chrome Driver；测试 feature 不会进入 release/NSIS 安装包。
