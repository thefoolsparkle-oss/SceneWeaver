...
function EntityCard({ entity }: { entity: Awaited<ReturnType<typeof listEntities>>[number] }) {
  ...
  const feedback = useMutation({
    mutationFn: ({ assetId, isPositive }: { assetId: string; isPositive: boolean }) => setEntityAssetFeedback(entity.id, assetId, isPositive),
    onSuccess: () => {
      references.refetch();
      matches.mutate();
    },
  });
  const chooseReference = async (isPositive: boolean) => {
    const e2eReferencePath = (import.meta as ViteImportMeta).env?.DEV
      ? (window as Window & { __SCENEWEAVER_E2E_ENTITY_REFERENCE_PATH__?: string }).__SCENEWEAVER_E2E_ENTITY_REFERENCE_PATH__
      : undefined;
    const path = e2eReferencePath ?? await open({ multiple: false, filters: [{ name: '图片', extensions: ['jpg', 'jpeg', 'png', 'webp', 'gif', 'bmp'] }] });
    if (typeof path === 'string') reference.mutate({ imagePath: path, isPositive });
  };
  return <section data-testid={`entity-card-${entity.id}`} className="rounded-xl border bg-white p-4 dark:bg-neutral-950"><p className="font-medium">{entity.name}</p><p className="text-xs text-neutral-500">{entity.entity_type}</p>{entity.aliases.length > 0 && <p className="mt-2 text-sm text-neutral-600">别名：{entity.aliases.join('、')}</p>}<p className="mt-2 text-xs text-neutral-500">正参考 {references.data?.filter((item) => item.is_positive).length ?? 0} 张 · 负参考 {references.data?.filter((item) => !item.is_positive).length ?? 0} 张</p><div className="mt-3 flex flex-wrap gap-2"><button data-testid={`entity-positive-reference-${entity.id}`} onClick={() => chooseReference(true)} disabled={reference.isPending} className="rounded border px-2 py-1 text-xs">添加正参考图</button><button data-testid={`entity-negative-reference-${entity.id}`} onClick={() => chooseReference(false)} disabled={reference.isPending} className="rounded border px-2 py-1 text-xs">添加负参考图</button><button data-testid={`entity-find-assets-${entity.id}`} onClick={() => matches.mutate()} disabled={matches.isPending} className="rounded bg-brand-600 px-2 py-1 text-xs text-white disabled:opacity-50">查找实体素材</button></div>{references.data && references.data.length > 0 && <ul className="mt-3 space-y-1 text-xs">{references.data.map((item) => <li key={item.id} className="flex items-center gap-2 rounded bg-neutral-50 px-2 py-1 dark:bg-neutral-900"><span className={item.is_positive ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-700 dark:text-rose-400'}>{item.is_positive ? '正参考' : '负参考'}</span><span className="min-w-0 flex-1 truncate" title={item.image_path ?? item.asset_id ?? ''}>{item.image_path ?? (item.asset_id ? `素材反馈：${item.asset_id}` : '无路径')}</span><button onClick={() => removeReference.mutate(item.id)} disabled={removeReference.isPending} className="rounded border px-1.5 py-0.5 hover:bg-white disabled:opacity-50 dark:hover:bg-neutral-800">移除</button></li>)}</ul>}{reference.isError && <p className="mt-2 text-xs text-red-600">{reference.error.message}</p>}{removeReference.isError && <p className="mt-2 text-xs text-red-600">移除失败：{removeReference.error.message}</p>}{feedback.isError && <p className="mt-2 text-xs text-red-600">反馈保存失败：{feedback.error.message}</p>}{matches.isError && <p className="mt-2 text-xs text-red-600">{matches.error.message}</p>}{matches.data && <div className="mt-4"><p className="mb-2 text-xs text-neutral-500">实体匹配素材 {matches.data.length} 项；将鼠标移到卡片上可标记“是实体”或“非实体”。</p><MediaGrid assets={matches.data} onEntityFeedback={(assetId, isPositive) => feedback.mutate({ assetId, isPositive })} /></div>}</section>;
}
